...
/**
 * Common parameters for Dingocoin networks.
 */
public abstract class AbstractDingocoinParams extends NetworkParameters implements AuxPoWNetworkParameters {
    /** Standard format for the DINGO denomination. */
    public static final MonetaryFormat DINGO;
    /** Standard format for the mDINGO denomination. */
    public static final MonetaryFormat MDINGO;
    /** Standard format for the Koinu denomination. */
    public static final MonetaryFormat KOINU;

    public static final int DIGISHIELD_BLOCK_HEIGHT = 5000; // Block height to use Digishield from
    public static final int AUX2_BLOCK_HEIGHT = 265000;
    public static final int AUXPOW_CHAIN_ID_OLD = 0x0062; // 98
    public static final int AUXPOW_CHAIN_ID = 0x0032; // 50
    public static final int DINGO_TARGET_TIMESPAN = 4 * 60 * 60;  // 4 hours per difficulty cycle, on average.
    public static final int DINGO_TARGET_TIMESPAN_NEW = 60;  // 60s per difficulty cycle, on average. Kicks in after block 5k.
    public static final int DINGO_TARGET_SPACING = 1 * 60;  // 1 minute per block.
    public static final int DINGO_INTERVAL = DINGO_TARGET_TIMESPAN / DINGO_TARGET_SPACING;
    public static final int DINGO_INTERVAL_NEW = DINGO_TARGET_TIMESPAN_NEW / DINGO_TARGET_SPACING;

    /** Currency code for base 1 Dingocoin. */
    public static final String CODE_DINGO = "DINGO";
    /** Currency code for base 1/1,000 Dingocoin. */
    public static final String CODE_MDINGO = "mDINGO";
    /** Currency code for base 1/100,000,000 Dingocoin. */
    ...
    static {
        DINGO = MonetaryFormat.BTC.noCode()
            .code(0, CODE_DINGO)
            .code(3, CODE_MDINGO)
            .code(7, CODE_KOINU);
        MDINGO = DINGO.shift(3).minDecimals(2).optionalDecimals(2);
        KOINU = DINGO.shift(7).minDecimals(0).optionalDecimals(2);
    }

    /** The string returned by getId() for the main, production network where people trade things. */
    public static final String ID_DINGO_MAINNET = "org.dingocoin.production";
    /** The string returned by getId() for the testnet. */
    public static final String ID_DINGO_TESTNET = "org.dingocoin.test";
    public static final String ID_DINGO_REGTEST = "org.dingocoin.regtest";
    ...
    protected Logger log = LoggerFactory.getLogger(AbstractDingocoinParams.class);
    public static final int DINGOCOIN_PROTOCOL_VERSION_AUXPOW = 70126;
    public static final int DINGOCOIN_PROTOCOL_VERSION_CURRENT = 70127;

    private static final Coin BASE_SUBSIDY   = COIN.multiply(500000);
    private static final Coin STABLE_SUBSIDY = COIN.multiply(10000);

    public AbstractDingocoinParams(final int setDiffChangeTarget) {
        super();
        genesisBlock = createGenesis(this);
        interval = DINGO_INTERVAL;
        newInterval = DINGO_INTERVAL_NEW;
        targetTimespan = DINGO_TARGET_TIMESPAN;
        newTargetTimespan = DINGO_TARGET_TIMESPAN_NEW;
     ...
     * Dingocoin after block 145k uses 60 seconds.
     */
    ...
    public Coin getMaxMoney() {
        // TODO: Change to be Dingo compatible
        return MAX_MONEY;
    }
    ...
    /** Dingocoin: Normally minimum difficulty blocks can only occur in between
     ...
     * the weird cases for Dingocoin such as testnet blocks which can be maximum
    ...
    public long calculateNewDifficultyTarget(StoredBlock storedPrev, Block nextBlock, BlockStore blockStore)
        ...
        // Dingocoin: Special rules for minimum difficulty blocks with Digishield
    ...
    public int getChainID() {
        if (height < AUX2_BLOCK_HEIGHT) {
            return AUXPOW_CHAIN_ID_OLD;
        } else {
            return AUX_POW_CHAIN_ID;
        }
    }
    ...
    public int getProtocolVersionNum(final ProtocolVersion version) {
        switch (version) {
            ...
            case CURRENT:
                return DINGOCOIN_PROTOCOL_VERSION_CURRENT;
            case MINIMUM:
            default:
                return DINGOCOIN_PROTOCOL_VERSION_AUXPOW;
        }
    }
    ...
    /**
     * Get the target time between individual blocks. Dingocoin uses this in its
